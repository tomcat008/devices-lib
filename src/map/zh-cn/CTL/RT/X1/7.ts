...
export = class CTL_RT_X1_DZQ extends CTL_RT_X1_Ts {
    constructor(){
        super();
        this.addPoint(new CountShowField(CTL_RT_X1_DZQ.KEY_BASE,CTL_RT.KEY_POINT_RAN_SHAO_QI,  "加热组",''));
        this.addPoint(new CountField(CTL_RT.KEY_POINT_Add_SHUI_BENG, "给水泵"));

        this.addPoint(new BaseInfoField(SdcSoftDevice.KEY_POINT_SYSTEM_STATUS, 3, 2, "控制器状态",'',CTL_RT_X1_DZQ.coms_status));
        this.addPoint(new BaseInfoField(SdcSoftDevice.KEY_POINT_RUN_LIFE, 53, 2, "运行时间","时"));
        this.addPoint(new CountShowField(CTL_RT_X1_DZQ.KEY_BASE,SdcSoftDevice.KEY_POINT_RUN_DAYS, "运行天数", "天"));
        this.addPoint(new CountShowField(CTL_RT_X1_DZQ.KEY_BASE,SdcSoftDevice.KEY_POINT_RUN_HOURS, "运行小时数", "时"));
        ...
        this.addPoint(new DeviceField("de_bushuibeng_shoudong/zidong_", 49, 2, "给水泵",1, CTL_RT_X1_DZQ.coms_auto));
